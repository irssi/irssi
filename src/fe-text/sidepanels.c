...
#include <irssip/src/fe-text/sidepanels.h>
...
static void position_tw(MAIN_WINDOW_REC *mw, SP_MAINWIN_CTX *ctx)
{
	...
	if (sp_auto_hide_right) {
		if (!aw || !aw->active || !aw->active->visible_name || !strchr(aw->active->visible_name, '#')) {
			show_right = FALSE;
		}
	}
	if (show_right) {
...
{
	(void)tw; (void)width; (void)height; (void)left;
}
...
static void draw_main_window_borders(MAIN_WINDOW_REC *mw)
{
	...
	if (ctx->left_tw && ctx->left_h > 0) {
		...
		for (int y = 0; y < ctx->left_h; y++) {
			gui_printtext_window_border(border_x, mw->first_line + mw->statusbar_lines_top + y);
		}
	}
...
static void handle_new_activity(WINDOW_REC *window, int data_level)
{
	...
	switch (data_level) {
		case DATA_LEVEL_NONE: data_level_name = "NONE"; break;
		case DATA_LEVEL_TEXT: data_level_name = "TEXT"; break;
		case DATA_LEVEL_MSG: data_level_name = "MSG"; break;
		case DATA_LEVEL_HILIGHT: data_level_name = "HILIGHT"; break;
		case DATA_LEVEL_EVENT: data_level_name = "EVENT"; break;
		default: data_level_name = "UNKNOWN"; break;
	}
...
static void reset_window_priority(WINDOW_REC *window)
{
	...
	if (state->current_priority > 0) {
		...
		sp_logf("RESET: Window %d priority reset from %d to 0 (opened)", window->refnum, state->current_priority);
		state->current_priority = 0;
	} else {
...
static inline void unformat(const unsigned char **ptr, int *color, unsigned int *fg24, unsigned int *bg24)
{
	switch (**ptr) {
...
static void draw_str_themed(TERM_WINDOW *tw, int x, int y, WINDOW_REC *wctx, int format_id, const char *text)
{
	...
	// sp_logf("THEME_DEBUG: format_id=%d text='%s' out='%s'", format_id, text ? text : "(null)", out ? out : "(null)");
 ...
 * };
 */
static void draw_str_themed_2params(TERM_WINDOW *tw, int x, int y, WINDOW_REC *wctx, int format_id, const char *param1, const char *param2)
{
	TEXT_DEST_REC dest;
	THEME_REC *theme;
	char *out, *expanded;
	...
	//        format_id, param1 ? param1 : "(null)", param2 ? param2 : "(null)", out ? out : "(null)");
...
static gint compare_window_sort_items(gconstpointer a, gconstpointer b)
{
	...
	if (w1->sort_group == 0) return -1; /* w1 is Notices */
	if (w2->sort_group == 0) return 1;  /* w2 is Notices */
	
	/* 2. Sort by server (alphabetically) */
	if (w1->server && w2->server) {
		net1 = w1->server->connrec && w1->server->connrec->chatnet ? w1->server->connrec->chatnet : 
		       (w1->server->connrec ? w1->server->connrec->address : "server");
		net2 = w2->server->connrec && w2->server->connrec->chatnet ? w2->server->connrec->chatnet : 
		       (w2->server->connrec ? w2->server->connrec->address : "server");
		server_cmp = g_ascii_strcasecmp(net1 ? net1 : "", net2 ? net2 : "");
		if (server_cmp != 0) return server_cmp;
	...
	return g_ascii_strcasecmp(w1->sort_key ? w1->sort_key : "", w2->sort_key ? w2->sort_key : "");
}
	...
	for (w = windows; w; w = w->next) {
		...
		if (win_name && g_ascii_strcasecmp(win_name, "Notices") == 0) {
			...
			net = sort_rec->server->connrec && sort_rec->server->connrec->chatnet ? 
			      ...
			      (sort_rec->server->connrec ? sort_rec->server->connrec->address : "server");
...
static gint ci_nick_compare(gconstpointer a, gconstpointer b)
{
	...
	if (!n1 || !n1->nick) return 1;
	if (!n2 || !n2->nick) return -1;
	...
	for (s = sort_list; s && row < height; s = s->next) {
		...
		if (win->refnum - 1 == ctx->left_selected_index) {
			...
			if (activity >= DATA_LEVEL_HILIGHT) {
				/* Check if this is a nick mention (has hilight_color indicating nick mention) */
			...
			switch (current_priority) {
				case 4: /* PRIORITY 4: Nick mention OR Query messages (magenta) */
					/* Use QUERY_MSG only for query windows, NICK_MENTION for channel mentions */
...
{
	const char *item_name = item ? item->visible_name : "NULL";
	const char *item_type = item ? (IS_QUERY(item) ? "QUERY" : IS_CHANNEL(item) ? "CHANNEL" : "OTHER") : "NONE";
...
static void sig_nicklist_changed(CHANNEL_REC *channel, NICK_REC *nick, const char *old_nick)
{
	...
	(void)channel; (void)nick; (void)old_nick;
	redraw_all();
}
...
static void sig_print_text(TEXT_DEST_REC *dest, const char *msg)
{
	...
	if (dest->level & (MSGLEVEL_JOINS | MSGLEVEL_PARTS | MSGLEVEL_QUITS | MSGLEVEL_KICKS | 
		...
		/* These are handled by message_join/part/quit/nick signals - skip to avoid duplicates */
		return;
	}
	...
	item_type = item ? (IS_QUERY(item) ? "QUERY" : IS_CHANNEL(item) ? "CHANNEL" : "OTHER") : "NONE";
...
static void sig_message_join(SERVER_REC *server, const char *channel, const char *nick, const char *address, const char *account, const char *realname)
{
	WINDOW_REC *window = window_find_item(server, channel);
	if (window) {
		/* Debug: Join events - tracks channel join activity */
		sp_logf("SIGNAL: message_join %s on %s (Window %d) active_win=%d", 
		        nick, channel, window->refnum, active_win ? active_win->refnum : -1);
		handle_new_activity(window, DATA_LEVEL_EVENT);
	}
	redraw_all();
}

static void sig_message_part(SERVER_REC *server, const char *channel, const char *nick, const char *address, const char *reason)
{
	WINDOW_REC *window = window_find_item(server, channel);
	if (window) {
		/* Debug: Part events - tracks channel part activity */
		sp_logf("SIGNAL: message_part %s from %s (Window %d) active_win=%d", 
		        nick, channel, window->refnum, active_win ? active_win->refnum : -1);
		handle_new_activity(window, DATA_LEVEL_EVENT);
	}
	redraw_all();
...
static void sig_message_quit(SERVER_REC *server, const char *nick, const char *address, const char *reason)
{
...
static void sig_message_nick(SERVER_REC *server, const char *newnick, const char *oldnick, const char *address)
{
			...
			if (x >= px && x < px+pw && y >= py && y < py+ph) {
				...
				for (s = sort_list; s; s = s->next) {
					if (idx++ == target_index) {
						...
						/* Debug: Window switches via mouse - tracks click navigation */
						sp_logf("MOUSE_CLICK: switching to window %d (was %d)", 
						        win ? win->refnum : -1, active_win ? active_win->refnum : -1);
						ctx->left_selected_index = target_index;
						if (win) window_set_active(win);
						redraw_one(mw);
						irssi_set_dirty();
						free_sorted_window_list(sort_list);
						return TRUE;
...
gboolean sidepanels_try_parse_mouse_key(unichar key)
{
	...
	if (reemit_guard) return FALSE; /* Don't process re-emitted keys */
	if (mouse_state == 0) {
		if (key == 0x1b) { 
			mouse_state = 1; 
			mouse_len = 0; 
