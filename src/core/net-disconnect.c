...
static void net_disconnect_remove(NET_DISCONNECT_REC *rec)
{
	...
	if (rec->handle != NULL)
		...
		net_disconnect_connection((GSocketConnection *) rec->stream);
	g_free(rec);
}
...
static gboolean sig_disconnect_source(GSocket *socket, GIOCondition cond, NET_DISCONNECT_REC *rec)
{
	...
	count = 0;
	do {
		ret = net_receive_connection(rec->stream, buf, sizeof(buf));
		if (ret == -1) {
			/* socket was closed */
			net_disconnect_remove(rec);
			return FALSE;
...
void net_disconnect_later(NET_SENDBUF_REC *handle)
{
	...
	if (handle->handle != NULL) {
		rec->handle = handle->handle;
		rec->tag =
		    i_input_add(rec->handle, I_INPUT_READ, (GInputFunction) sig_disconnect, rec);
	} else if (handle->connection != NULL) {
		// GInputStream *in;
		GSocket *socket;
		GSource *source;

		socket = g_socket_connection_get_socket(handle->connection);
		...
		rec->stream = (GIOStream *) handle->connection;
