...
static int cmd_tag;
static char *rejoin_channels_mode;
...
char *irc_server_get_channels(IRC_SERVER_REC *server)
{
	...
	GString *chans, *keys;
	char *ret;
	rejoin_channels_mode = settings_get_str("rejoin_channels_on_reconnect");

	if (rejoin_channels_mode == NULL || (g_ascii_strcasecmp(rejoin_channels_mode, "on") != 0 && g_ascii_strcasecmp(rejoin_channels_mode, "on") != 0 && g_ascii_strcasecmp(rejoin_channels_mode, "on") != 0))
	...
	for (tmp = server->channels; tmp != NULL; tmp = tmp->next) {
		...
		CHANNEL_SETUP_REC *setup = channel_setup_find(channel->name,
							channel->server->connrec->chatnet);
		if ((setup != NULL && setup->autojoin && g_ascii_strcasecmp(rejoin_channels_mode, "auto") == 0) || g_ascii_strcasecmp(rejoin_channels_mode, "on") == 0) {
	...
	for (tmp = server->rejoin_channels; tmp != NULL; tmp = tmp->next) {
		...
		CHANNEL_SETUP_REC *setup = channel_setup_find(rec->channel,
							server->tag);
